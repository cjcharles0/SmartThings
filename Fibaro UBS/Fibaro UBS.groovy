...
private void createChildDevices(){
	log.debug "Adding Child Devices if not already added - first creating Contact1 child"
    try {
    ...
    log.debug "Now creating Contact2 child if not already added"
    try {
...
private void createChildTempDevices() {
    ...
    for (def i = 1; i <= 4; i++) {
    	try {
        	...
        	if (device.currentValue("temperature${i}") != null) {
                ...
                if (currentchild == null) {
                    addChildDevice("smartthings/testing", "Simulated Temperature Sensor", "${device.deviceNetworkId}-temperature${i}", device.hub.id,
                        [completedSetup: true, name: "${device.displayName} (Temp${i})", label: "${device.displayName} (Temp${i})", isComponent: false])
                }
            }
            else {
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
	...
	if (cmd.commandClass == 32) {
    	def currentcontact = "contact" + cmd.sourceEndPoint
        def currentstate
		if (cmd.parameter == [0]) {
        	currentstate = "closed"
		}
        ...
        if (currentcontact != null) {
			//If not null then we have found either contact1 or contact2, hence try to send to the child device aswell
            try {
